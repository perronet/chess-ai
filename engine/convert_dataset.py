...
if __name__ == "__main__":
    ...
    for i, df_chunk in enumerate(pd.read_csv(setup.DATASET_DIR+setup.DATASET, chunksize=CHUNK_SIZE)):
        ...
        df_converted["label"] = df_chunk.apply(lambda row: parse.convert_stockfish_eval(row.FEN, row.Evaluation), axis=1)
        
        df_converted.to_csv(out_path, mode="a", header=not os.path.exists(out_path))
        del df_converted
